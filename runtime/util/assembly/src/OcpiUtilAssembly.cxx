...
namespace OCPI {
  namespace Util {
    ...
    parseConnection(ezxml_t ix, Assembly &a, const PValue *params) {
      const char *err, *c, *ci = NULL; // quiet compiler warning
      const char *e = ezxml_cattr(ix, "external");
      if ((c = ezxml_cattr(ix, "connect")) || (ci = ezxml_cattr(ix, "connectinput"))) {
	if (e)
	  return esprintf("Instance %s has both \"external\" and \"connect\" attributes", m_name.c_str());
        unsigned n;
        ...
        return esprintf("Instance %s has buffersize attribute without connect attribute",
                        m_name.c_str());
      if (e && (err = a.addExternalConnection(ix, m_ordinal, e, params)))
	return err;
#if 1
