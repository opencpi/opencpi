...
#define EPNAME "ocpi-scif-dma"
	...
	: DT::SmemServices(ep), 
	  MemBlockMgr(ep.size, SCIF_FLAG_MEM_SIZE),
	...
	if ( retries <= 0 ) {
	  snprintf(eb,EB_SIZE,"scif_connect failed with error %d\n", errno);
	  throw OU::Error(eb);
	}
      }
	...
	snprintf(eb,EB_SIZE, "scif_register failed with error : %s\n",  strerror(errno));
	throw OU::Error(eb);
      }
...
#define TRACE_XFERS  
#ifdef TRACE_XFERS
      printf("scif: copying %d bytes from 0x%llx to 0x%llx", transfer->nbytes,transfer->src_off,transfer->dst_off);
#endif
	...
	*((int*)parent().m_sourceSmb->map((DtOsDataTypes::Offset)m_addr,4)) = 0;
	...
	if ( (ret=scif_fence_signal( parent().m_sourceSmb->epn(CONNECT), VA_GEN_MIN + m_addr, 1, VA_GEN_MIN + transfer->dst_off, 
	  ...
	  snprintf(eb, EB_SIZE, "scif_fence_signal failed with err = %d,error %s\n", ret,strerror(errno));
	  printf( eb );
	  throw OU::Error(eb);	  
	}
	...
	if ( (ret=scif_writeto ( parent().m_sourceSmb->epn(CONNECT), VA_GEN_MIN + transfer->src_off, transfer->nbytes,
	  ...
	  snprintf(eb, EB_SIZE, "scif_writeto failed with err = %d,error %s\n", ret,strerror(errno));
	  printf( eb );
	  throw OU::Error(eb);
	}
      }
