...
namespace OCPI {
  namespace Container {
    ...
    bool Container::supportsImplementation(OU::Worker &i) {
        static std::string our_ocpi_version; // AV-2453
        static bool version_override = false;
        if (our_ocpi_version.empty()) {
            char *envstr;
            ocpiCheck(asprintf(&envstr, "%d.%d", OCPI_VERSION_MAJOR, OCPI_VERSION_MINOR) > 0);
            our_ocpi_version = envstr;
            free(envstr);
            const char *ovr = getenv("OCPI_ALLOW_VERSION_MISMATCH");
            version_override = (ovr and (0 == strncmp(ovr, "1", 1)));
            ocpiInfo("Artifact version checking is %sin effect. %s",
                version_override?"NOT ":"",
                version_override?"":"Set OCPI_ALLOW_VERSION_MISMATCH to 1 to allow.");
    }
      bool ok =
	m_model == i.model() &&
    ...
    // Checking AV version is not part of the "ok" above because we can override with environment and want to warn.
    if (ok and not version_override) {
        ok = (our_ocpi_version == i.attributes().version());
        if (not ok)
            ocpiBad("Rejected '%s' ONLY because of artifact version mismatch ('%s' vs. expected '%s')%s",
                i.cname(), i.attributes().version().c_str(), our_ocpi_version.c_str(),
                (OCPI::OS::logWillLog(OCPI_LOG_INFO))?"":" (try increasing log level)");
    }
      ocpiInfo("vs. container %s (%u) model %s os %s version %s arch %s platform %s dynamic %u ver %s ==> %s",
		name().c_str(), m_ordinal, m_model.c_str(), m_os.c_str(), m_osVersion.c_str(),
	       m_arch.c_str(), m_platform.c_str(), m_dynamic, our_ocpi_version.c_str(), ok ? "accepted" : "rejected");
      return ok;
    }
