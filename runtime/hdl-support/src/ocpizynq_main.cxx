...
bool get_is_ultrascale_plus(volatile USP_AXI_HP *axi_hp) {
  bool ret = false;
  if (axi_hp) {
    // i.e. S_AXI_HP0_FPD AFIFM RDDEBUG register's bitfield's
    // AFI_VERSION == 'Zynq 8 Series', ref:
    // https://www.xilinx.com/html_docs/registers/ug1087/ug1087-zynq-ultrascale-registers.html
    ret = (axi_hp->afifm->rddebug & 0x40000000) == 0x40000000;
  }
  if (ret) {
...
mymain(const char **argv) {
  ...
  if (cmd == "test")
    ...
    bool is_ultra_scale_plus = get_is_ultrascale_plus(axi_hp);
    if (is_ultra_scale_plus) { // implies axi_hp != 0
      volatile USP_AFIFM *afifm = axi_hp->afifm;
      if (cmd == "clocks") {
        printf("clocks command is net yet supported on UltraScale+ architectures\n");
        return 1;
      }
      else if (cmd == "axi_hp") {
