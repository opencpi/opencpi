...
echo PATH is: $PATH
echo Executing remote configuration command: $1 >&2
case $1 in
  start)
      ...
      nohup ocpiserve -v -l9 -p $(<port) > $log 2>&1 &
      pid=$!
      sleep 1
	  ...
	  wait $pid
	  echo Could not start ocpiserve: exit status $? >&2
	  exit 1
      fi;;
  stop)
   ...
   status)
    ...
    echo Server is running in pid: $pid >&2;;
esac
