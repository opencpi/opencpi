...
for op in $*; do
  case $op in
    ...
    load)
      ...
      tar -c -z -H -f $tmpdir/tar.tgz /etc/localtime scripts/ocpi_${os}_driver scripts/ocpi_server.sh \
	  $platform/lib/*.ko $platform/lib/*.rules ${driverlibs[*]}
	  $platform/bin/ocpidriver $platform/bin/ocpiserve $platform/system.xml
	...
	;;
    unload|remove)
	do_ssh "[ -d $rdir ] && rm -r -f $rdir";;
    start)
	do_ssh "$checkdir; cd $rdir && ./ocpi_server.sh start";;
    log)
	do_ssh "$checkdir; cd $rdir && ./ocpi_server.sh log";;
    stop)
	do_ssh "$checkdir; cd $rdir && ./ocpi_server.sh stop";;
    stop_if)
	do_ssh "test ! -d $rdir || (cd $rdir && ./ocpi_server.sh stop_if)";;
    status)
	do_ssh "$checkdir; cd $rdir && ./ocpi_server.sh status";;
  esac
done
