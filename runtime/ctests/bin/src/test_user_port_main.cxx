...
static OCPI::API::ExternalPort *external_input_port;
static OCPI::API::ExternalPort *external_output_port;
...
static bool run_ap_test(std::vector<CApp>& ca, std::vector<CWorker*>& workers, int mode)
{
  ...
  while ( count > 0 ) {
    ...
    // For this test we have interjected ourselves between the producer and consumer.  So in this loop
    ...
    if ( in_buffer ) {
      ...
      if ( eb ) {
        ...
        eb->put(in_length,  in_opCode, in_endOfData );
        in_buffer->release();
        in_buffer = NULL;
      }
    }
      ...
      in_buffer =
        external_input_port->getBuffer(in_data, in_length,  in_opCode, in_endOfData );
    }
