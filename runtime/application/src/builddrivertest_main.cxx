...
bool doTestDrivers() {
  ...
  for (size_t pos(0), lastpos(0);
       std::string::npos != (pos = DriverList.find(' ', lastpos)); lastpos = pos + 1) {
    const std::string Driver(".libs/" + DriverList.substr(lastpos, pos - lastpos) + ".so");
    std::cout << "Loading Driver '" << Driver << "': ";
    if (Driver.find("libocpi_ocl") != std::string::npos && system(ocpiocl.c_str())) {
      std::cout << "  Skipping OCL driver since OCL is not available" << std::endl;
      continue;
    }
