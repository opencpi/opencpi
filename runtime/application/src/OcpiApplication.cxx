  ...
  namespace API {
	...
	*pn++ = uProp.m_ordinal; // remember position in property list
	...
	for (unsigned m = 0; m < i->m_nCandidates; m++) {
	  ...
	  ocpiInfo("Checking implementation %s model %s os %s version %s arch %s platform %s dynamic %u ver %s",
	...
	if (!sum) {
	  if (m_verbose) {
	    ...
	    for (unsigned m = 0; m < i->m_nCandidates; m++) {
	      ...
	      fprintf(stderr, "  Name: %s, Model: %s, Arch: %s, Platform: %s%s%s, Version: %s, File: %s\n",
		      mImpl.cname(),
		      mImpl.model().c_str(),
		      ...
		      mImpl.attributes().version().c_str(),
