...
class TestUtilization(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if subprocess.call("mkdir -p " + UTIL_PROJ, shell=True) != 0:
           raise ocpiutil.OCPIException("mkdir failed to create the utililzation test project directory")
        with ocpiutil.cd(UTIL_PROJ):
            if subprocess.call("tar xf ../" + UTIL_PROJ + ".tgz", shell=True) != 0:
                raise ocpiutil.OCPIException("could not unpack tar file for utililzation test project")
