...
class BiasFGM_ccWorker : public BiasFGM_ccWorkerBase {
  ...
  MyTaskC tasks[TC];
  RCCResult release() {
    printf("bias worker waiting to join\n");
    this->join(true);
    printf("bias worker done joining \n");
    return RCC_OK;
  }
  RCCResult stop() {
  ...
  RCCResult run(bool /*timedout*/) {
    ...
    if ( in.length() < 1 ) {
      for (unsigned n = in.length(); n; n--) // n is length in sequence elements of input
