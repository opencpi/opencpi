 ...
 * 2/ compile:
 *
 *  gcc -c -fPIC -m32 inode64.c                                                           -ldl
 *
 * 3/ link
 *
 * ld -shared -melf_i386 --version-script vers -o inode64.so inode64.o                    /usr/lib/libdl.so
 *
...
// Modifications by OpenCPI: Intercept the "true" 64-bit versions and enforce 32-bit inodes there as well.
...
int (*orig_fxstat64)(int ver, int fd, struct stat64 *buf);

int __fxstat64 (int ver, int fd, struct stat64 *buf){
//   fprintf(stderr,"Hacked __fxstat64 called\n");
  int i = orig_fxstat64(ver,fd,buf);
  if (i) return i;
  if (buf->st_ino >> 32) {
    const uint64_t orig=buf->st_ino;
    buf->st_ino=((buf->st_ino)^((buf->st_ino)>>32)) & 0xFFFFFFFF;
    fprintf(stderr,"__fxstat64: inode %016llx => %08lx\n", orig, (uint32_t) (buf->st_ino));
  }
  return 0;
}
...
int (*orig_xstat64)(int ver, const char *path, struct stat64 *buf);

int __xstat64 (int ver, const char *path, struct stat64 *buf){
//   fprintf(stderr,"Hacked __xstat64 called\n");
  int i = orig_xstat64(ver,path,buf);
  if (i) return i;
  if (buf->st_ino >> 32) {
    const uint64_t orig=buf->st_ino;
    buf->st_ino=((buf->st_ino)^((buf->st_ino)>>32)) & 0xFFFFFFFF;
    fprintf(stderr,"__xstat64: inode %016llx => %08lx\n", orig, (uint32_t) (buf->st_ino));
  }
  return 0;
}
...
int (*orig_lxstat64)(int ver, const char *path, struct stat64 *buf);

int __lxstat64 (int ver, const char *path, struct stat64 *buf){
//   fprintf(stderr,"Hacked __lxstat64 called\n");
  int i = orig_lxstat64(ver,path,buf);
  if (i) return i;
  if (buf->st_ino >> 32) {
    const uint64_t orig=buf->st_ino;
    buf->st_ino=((buf->st_ino)^((buf->st_ino)>>32)) & 0xFFFFFFFF;
    fprintf(stderr,"__lxstat64: inode %016llx => %08lx\n", orig, (uint32_t) (buf->st_ino));
  }
  return 0;
}
...
_init(void)
{
  // fprintf(stderr,"Loading inode hacks.\n");
  orig_fxstat64 = dlsym(RTLD_NEXT, "__fxstat64");
  orig_xstat64 = dlsym(RTLD_NEXT, "__xstat64");
  orig_lxstat64 = dlsym(RTLD_NEXT, "__lxstat64");
}
