...
namespace {
  ...
  class MyNamingContext_impl :
    public OCPI::CORBAUtil::WaitForNameServiceBinding::NamingContextBase,
...
OCPI::CORBAUtil::WaitForNameServiceBinding::NamingContextBase::
NamingContextBase (const CosNaming::Name & expectedBinding)
  throw ()
  : m_haveBinding (false),
    m_expectedBinding (expectedBinding)
{
  ocpiAssert (m_expectedBinding.length());
}
...
OCPI::CORBAUtil::WaitForNameServiceBinding::
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        ocpiAssert (!CORBA::is_nil (m_context));
      }
      catch (...) {
