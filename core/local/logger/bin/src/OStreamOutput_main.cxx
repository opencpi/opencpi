...
namespace OStreamOutputTests {
   ...
   * same logic as in CpiLoggerOStreamOutput.cxx
   * ----------------------------------------------------------------------
   */
  ...
  public:
    Test1 ()
      : CPI::Util::Test::Test ("Writing a \"Hello World\" log message")
    {
    }
  ...
  threadSafetyTestThread1 (void * opaque)
  {
    CPI::Logger::Logger * logger =
      reinterpret_cast<CPI::Logger::Logger *> (opaque);
  ...
  public:
    Test2 ()
      : CPI::Util::Test::Test ("Thread safety")
    {
    }
    ...
    void run ()
    {
      std::ostringstream out;
      CPI::Logger::OStreamOutput logger (out);
      CPI::Logger::Logger * pl = &logger;
      logger.setProducerId ("02-OStreamOutput");

      CPI::OS::ThreadManager tm1 (threadSafetyTestThread1, pl);
...
{
  CPI::Util::Test::Suite tests ("OStreamOutput tests");
  int n_failed;
