...
/**
 * \mainpage The CPI::Logger Logging Infrastructure
 *
 ...
 * CPI::Logger::Logger base class, to all functions and classes that
 ...
 *
 * CPI::Logger::Logger &mdash; The abstract base class.
 *
 ...
 *
 * CPI::Logger::NullOutput &mdash; Send log messages to the "null device".\n
 * CPI::Logger::OStreamOutput &mdash; Send log messages to any std::ostream.\n
 *
 ...
 *
 * CPI::Logger::PrefixInserter &mdash; Add a prefix to each log message.\n
 * CPI::Logger::Tee &mdash; Send log messages to multiple loggers.\n
 * CPI::Logger::Fallback &mdash; Fall back to another logger if the first one fails.\n
 *
 * \section other Others
 *
 * CPI::Logger::OStreamAdapter &mdash; Adapt legacy code that logs to an std::ostream.\n
 *
 * \section debug Debug Logger
 *
 * CpiLoggerDebugLogger.h &mdash; Conditionally log debugging messages.\n
 *
 * Depending on compile-time definition of the the NDEBUG preprocessor
 * symbol, either CPI::Logger::DebugEnabled::DebugLogger or
 * CPI::Logger::DebugDisabled::DebugLogger is imported into the
 * CPI::Logger namespace and becomes CPI::Logger::DebugLogger.
 * Because doxygen does not interpret the C++ "using" statement,
 * CPI::Logger::DebugLogger does not show in this documentation.
 * Please refer to the header file CpiLoggerDebugLogger.h for more
 * information.
 *
 * \section notes Notes
 *
...
#ifndef CPILOGGERLOGGER_H__
#define CPILOGGERLOGGER_H__
 ...
 * \file
 *
 * \brief The CPI::Logger::Logger base class and helpers.
 */
...
namespace CPI {
  namespace Logger {
    ...
    namespace Level {
       ...
       *
       * See CPI::Logger::LogLevel for user-defineable log levels.
       *
     ...
     * names for levels 1 to 9 (see CPI::Logger::Level::LwLogLevel).
     ...
     *    operator) a CPI::Logger::LogLevel object or one of the predefined
     *    CPI::Logger::Level::LwLogLevel enumeration values.
     * -# Optionally, set the <em>producer name</em> by emitting (using the
     *    \<\< operator) a CPI::Logger::ProducerName object.
    ...
    public:
       ...
       * operators for CPI::Logger::Level::LwLogLevel or
       * CPI::Logger::LogLevel.
       */
       ...
       * output operator for CPI::Logger::ProducerName.
       */
...
inline
CPI::Logger::LogLevel::LogLevel (unsigned short logLevel)
{
  m_level = logLevel;
}

inline
CPI::Logger::ProducerName::ProducerName (const char * producerName)
  : m_producerName (producerName)
{
}

inline
CPI::Logger::ProducerName::ProducerName (const std::string & producerName)
...
inline
void
CPI::Logger::Logger::setLogLevel (unsigned short logLevel)
{
...
inline
void
CPI::Logger::Logger::setProducerId (const char * producerId)
{
...
inline
void
CPI::Logger::Logger::setProducerName (const char * producerName)
{
...
inline
CPI::Logger::Logger &
operator<< (CPI::Logger::Logger & logger, CPI::Logger::Level::LwLogLevel logLevel)
{
...
inline
CPI::Logger::Logger &
operator<< (CPI::Logger::Logger & logger, const CPI::Logger::LogLevel & logLevel)
{
...
inline
CPI::Logger::Logger &
operator<< (CPI::Logger::Logger & logger, const CPI::Logger::ProducerName & producerName)
{
