...
CPI::Logger::OStreamOutput::OStreamOutputBuf::
...
CPI::Logger::OStreamOutput::OStreamOutputBuf::~OStreamOutputBuf ()
{
  if (!m_first) {
    m_out->flush ();
  }
...
void
CPI::Logger::OStreamOutput::OStreamOutputBuf::setLogLevel (unsigned short logLevel)
{
  m_lock.lock ();
  m_locked = true;
  m_logLevel = logLevel;
...
void
CPI::Logger::OStreamOutput::OStreamOutputBuf::setProducerId (const char * producerId)
{
  m_producerId = producerId;
}

void
CPI::Logger::OStreamOutput::OStreamOutputBuf::setProducerName (const char * producerName)
{
  cpiAssert (m_locked);
  cpiAssert (m_first);
...
int
CPI::Logger::OStreamOutput::OStreamOutputBuf::sync ()
{
  if (!m_first) {
    cpiAssert (m_locked);
    *m_out << std::endl;
...
CPI::Logger::OStreamOutput::OStreamOutputBuf::overflow (int_type c)
{
  cpiAssert (m_locked);
...
std::streamsize
CPI::Logger::OStreamOutput::OStreamOutputBuf::xsputn (const char * data, std::streamsize count)
{
  cpiAssert (m_locked);
...
CPI::Logger::OStreamOutput::OStreamOutput (std::ostream & out)
  : Logger (m_obuf),
    m_obuf (&out, false)
{
}

CPI::Logger::OStreamOutput::OStreamOutput (std::ostream * out, bool adopt)
  : Logger (m_obuf),
    m_obuf (out, adopt)
{
}

CPI::Logger::OStreamOutput::~OStreamOutput ()
{
}
