/*
 * Abstact:
 *   These are the class implementations for the CPI expression parser
 *
...
CPI::ParsedExpression* CPI::Parser::parseExpression( const char* exp )
{
  CPI::ParsedExpression* pexp = new CPI::ParsedExpression( cb_ );
...
double CPI::ParsedExpression::evaluate()
{
  double value=0;
...
CPI::Tokenizer::Tokenizer( const char* str,DefineExpVarInterface* cb )
...
void CPI::Tokenizer::operator ++(int)
{
...
CPI::Operation* CPI::Tokenizer::token()
{
  // map the basic type
...
CPI::ParsedExpression::~ParsedExpression()
{
  delete tokenizer_;
...
// For debug
void CPI::ParsedExpression::printOrigList()
{
  Operation* op = first_;
  while( op ) {
    op->printSelf();
...
void CPI::ParsedExpression::printWorkingList()
{
  Operation* op = first_;
  while( op ) {
    op->printSelf();
...
void CPI::ParsedExpression::addOperation( Operation* op )
{
  if ( first_ == NULL ) {
    first_ = op;
    last_ = op;
...
class MyVarDefiner : public CPI::DefineExpVarInterface {
...
void run_exp_tests()
{
  ...
  try {
    ...
    CPI::Parser *exp_parser = new CPI::Parser( my_var_definer );
