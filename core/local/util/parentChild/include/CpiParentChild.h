...
namespace CPI {
  namespace Util {
	...
	// call a function on all children, stopping if it returns true
	// it should not add or remove anything.
	TChild *findChild( const char* arg ) {
	  for (ChildI<TChild> *cp = myChildren; cp; cp = cp->next) {
	    if ( static_cast<TChild *>(cp)->operator==(arg) ) {
...
namespace CPI {
  ...
  namespace Util {
	...
	myParent(NULL)  
	{
      ...
      ~Child<TParent,TChild> () {
	  if ( myParent ) {
	    myParent->releaseChild(static_cast<TChild&>(*this));
	  }
	}
	virtual bool operator==(const char* rhs_n ) const{return m_cname == rhs_n;}
    };
