...
const std::string
CPI::Util::Http::ClientStream::userAgent = "mcClient/0.1";

const std::string
CPI::Util::Http::ClientStream::defaultContentType = "application/octet-stream";

const std::string
CPI::Util::Http::ClientStream::defaultCharset = "iso-8859-1";
...
CPI::Util::Http::Redirection::Redirection (int c, const std::string & x)
  : statusCode (c), newLocation (x)
{
}

CPI::Util::Http::Redirection::operator const std::string & () const
{
  return newLocation;
}
...
receiveStatus ()
  throw (std::string)
{
  m_status = CPI::Util::Misc::readline (m_inConn, MAX_LENGTH_OF_HEADER_LINE);
  ...
  throw (std::string)
{
  ...
  while (m_inConn->good() && headerLine.length()) {
    if (++count > MAX_NUM_OF_HEADER_LINES) {
      ...
      reason += CPI::Util::Misc::integerToString (MAX_NUM_OF_HEADER_LINES);
      reason += " header lines";
      throw reason;
    }
    ...
    if (!CPI::Util::Http::isToken (fieldName)) {
      std::string reason = "illegal header line: \"";
      reason += headerLine;
      reason += "\"";
      throw reason;
  ...
  throw ()
{
  ...
  // cpiAssert (!m_connected);
}

void
CPI::Util::Http::ClientStream::
connect (const CPI::Util::Uri & uri)
  throw (std::string)
{
  if (m_connected) {
    reset ();
  ...
  throw (std::string, Redirection, ClientError, ServerError)
{
  ...
  if (contentLength != static_cast<unsigned long long> (-1)) {
    headers["Content-Length"] =
      CPI::Util::Misc::unsignedToString (contentLength);
  }
  ...
  throw (std::string, Redirection, ClientError, ServerError)
{
  ...
  return CPI::Util::Http::parseHttpDate ((*it).second);
}
