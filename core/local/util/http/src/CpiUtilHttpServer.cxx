...
CPI::Logger::NullOutput
CPI::Util::Http::Server::
g_myDefaultLogger;

CPI::Logger::ProducerName
CPI::Util::Http::Server::
g_myProducerName ("CPI::Util::Http::Server");
...
void
CPI::Util::Http::Server::
setLogger (CPI::Logger::Logger * out)
  throw ()
{
  m_logger = out;
  ...
  throw ()
{
  ...
  std::string headerLine = CPI::Util::Misc::readline (m_inConn);
  int count=0;
  ...
  throw ()
{
  ...
  if (ims != m_requestHeaders.end() && lastModified != (time_t) -1) {
    ...
    try {
      oldTimestamp = CPI::Util::Http::parseHttpDate ((*ims).second);
    }
    catch (...) {
