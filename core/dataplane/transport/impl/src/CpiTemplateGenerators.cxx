...
/*
 * Abstact:
 *   This file contains the CPI template generator implementation.
 *
 * Revision History: 
 * 
...
#define FORMAT_TRANSFER_EC_RETHROW( sep, tep )                                \
  ...
  throw CPI::Util::EmbeddedException ( UNABLE_TO_CREATE_TX_REQUEST, buf );
...
create( Transport* transport, PortSet* output, PortSet* input, TransferController* cont )
{
  ...
  for ( CPI::OS::uint32_t s_n=0; s_n<output->getPortCount(); s_n++ ) {
  ...
  for ( CPI::OS::uint32_t t_n=0; t_n<input->getPortCount(); t_n++ ) {
  ...
  if ( ! generated ) {
    cpiAssert( ! "PROGRAMMING ERROR!! no templates were generated for this circuit !!\n");
  }
                                                               ...
                                                               TransferController* cont )
{
  ...
  for ( int s_buffers=0; s_buffers<n_s_buffers; s_buffers++ ) {
    ...
    for ( int t_buffers=0; t_buffers<n_t_buffers; t_buffers++ ) {
      ...
      for ( CPI::OS::uint32_t ns=0; ns<s_port->getPortSet()->getPortCount(); ns++ ) {
        Port* next_sp = 
...

{
  ...
  // We need to create the transfers to tell all of our shadow ports that a buffer
  // became available, 
  CPI::OS::uint32_t n_t_buffers = input->getBufferCount();

  // We need a transfer template to allow a transfer for each input buffer to its
  // associated shadows
  for ( CPI::OS::uint32_t t_buffers=0; t_buffers<n_t_buffers; t_buffers++ ) {
...
createOutputTransfers(CPI::DataTransport::Port* s_port, 
  ...
  for ( int s_buffers=0; s_buffers<n_s_buffers; s_buffers++ ) {
    ...
    for ( n=0; n<n_t_ports; n++ ) {
      ...
      for ( int t_buffers=0; t_buffers<n_t_buffers; t_buffers++ ) {
        ...
        // Create a template
        CpiTransferTemplate* temp = new CpiTransferTemplate(2);
...
XferRequest* TransferTemplateGeneratorPattern3::addTransferPreState( TDataInterface& tdi)
{
  ...
  for ( CPI::OS::uint32_t n=0; n<sps->getPortCount(); n++ ) {
                      ...
                      TransferController* cont  )
{
  ...
  // input buffer for this pattern.
  int sequence;
  CpiTransferTemplate *root_temp=NULL, *temp=NULL;
  for ( int s_buffers=0; s_buffers<n_s_buffers; s_buffers++ ) {
      ...
      for ( int transfer_count=0; transfer_count<n_transfers_per_output_buffer; 
        ...
        for ( int t_gated_buffer=0; t_gated_buffer<n_t_buffers+1; t_gated_buffer++ ) {
          ...
          if ( (transfer_count == 0) && (t_gated_buffer == 0)) {
            ...
            temp = new CpiTransferTemplate(4);
            root_temp = temp;
