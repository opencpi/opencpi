...
{                         
  ...
  for ( OCPI::OS::uint32_t x=0; x<MAX_PCONTRIBS; x++ ) {
    for ( OCPI::OS::uint32_t y=0; y<MAX_BUFFERS; y++ ) {
      for ( OCPI::OS::uint32_t z=0; z<MAX_PCONTRIBS; z++ ) {
        for ( OCPI::OS::uint32_t zz=0; zz<MAX_BUFFERS; zz++ ) {
          for ( OCPI::OS::uint32_t bc=0; bc<2; bc++) {
            for ( OCPI::OS::uint32_t ts=0; ts<2; ts++) {
                                    ...
                                    )
{
  ...
  m_FillQPtr++ ;
  if ( m_FillQPtr >= (OCPI::OS::int32_t)port->getBufferCount() ) {
    m_FillQPtr = 0;
  }
}
                                     ...
                                     )
{
  ...
  m_EmptyQPtr++;
  if ( m_EmptyQPtr >= (OCPI::OS::int32_t)port->getBufferCount() ) {
    m_EmptyQPtr = 0;
  }
}
...
void TransferController::freeAllBuffersLocal( OCPI::DataTransport::Port* port  )
{
...
void TransferController::consumeAllBuffersLocal( OCPI::DataTransport::Port* port )
{
  ocpiAssert( port->isOutput() );
  for ( unsigned int n=0; n<port->getBufferCount(); n++ ) {
    port->getBuffer(n)->markBufferEmpty();
...
int TransferController4::produce( Buffer* b, bool bcast )
{
  ...
  OCPI::OS::int32_t n_pending = buffer->getPendingTransferCount();
  if ( ! n_pending ) {
