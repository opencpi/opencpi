...
/*
 * Abstact:
 *   This file contains the implementation for the CPI buffer class.
 *
 * Revision History: 
 *
 *    05/18/09 
 ...
 * Constructors
 *********************************/
Buffer::Buffer( CPI::DataTransport::Port* port, CPI::OS::uint32_t tid )
...
{
  cpiAssert( dependent_buffer );
  cpiAssert( (CPI::OS::uint32_t)dependent_buffer->getPort()->getPortId() < m_dependentZeroCopyPorts.size() );
...
void 
Port::
addBuffer( CPI::DataTransport::Buffer* buf )
{
...
BufferMetaData* 
Buffer::
getMetaDataByPortId( CPI::OS::uint32_t id )
{
  return &m_sbMd[0][id];
}
