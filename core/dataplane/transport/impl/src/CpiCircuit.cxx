...
// our init flag
CPI::OS::uint32_t CPI::DataTransport::Circuit::m_init=0;
     ...
     m_outputPs(0), m_inputPs(0),  m_metaData(connection) ,m_portsets_init(0)
{
  ...
  if ( t->m_transportGlobal->m_Circuitinit == false ) {
    ...
    m_transport->m_transportGlobal->m_templateGenerators[DataDistributionMetaData::parallel][DataDistributionMetaData::parallel]
      [DataPartitionMetaData::INDIVISIBLE][DataPartitionMetaData::INDIVISIBLE] 
      [true] [CPI::RDT::ActiveMessage] [CPI::RDT::ActiveMessage] 
      = m_transport->m_transportGlobal->m_gen_pat1;
  ...
  for ( CPI::OS::uint32_t psc=0; psc<m_metaData->m_portSetMd.size(); psc++ ) {
...
{
  CPI::DataTransport::ConnectionMetaData* cmd = getConnectionMetaData();
  CPI::DataTransport::PortSetMetaData* psmd = static_cast<CPI::DataTransport::PortSetMetaData*>(cmd->m_portSetMd[0]);
...
~Circuit()
{
  ...
  for ( CPI::OS::uint32_t n=0; n<this->getOutputPortSet()->getPortCount(); n++ ) {
    printf("spc = %d\n", this->getOutputPortSet()->getPortCount());
    CPI::DataTransport::Port* port = 
      static_cast<CPI::DataTransport::Port*>(this->getOutputPortSet()->getPortFromIndex(n));
    printf("port = %d\n", port );
    if ( ! port->isShadow() ) {
...
updateInputs( void* data )
{
  ...
  CPI::DataTransport::Port* port = 
    static_cast<CPI::DataTransport::Port*>(this->getInputPortSet(0)->getPortFromOrdinal(update->receiverPortId));
  ...
  CPI::DataTransport::PortSet* ps = getOutputPortSet();
  CPI::OS::uint32_t i;
  CPI::OS::uint32_t sports=0, tports=0;
  for ( i=0; i<ps->getPortCount(); i++ ) {
    ps->getPort(i)->update();
    sports++;
  }
  for ( CPI::OS::uint32_t m=0; m<getInputPortSetCount(); m++) {
...
updateInputs()
{
  CPI::DataTransport::Port* output_port = 
    static_cast<CPI::DataTransport::Port*>(getOutputPortSet()->getPortFromIndex(0));
  ...
  CPI::Util::VList& tps = getInputPortSets();
  PortSet* ps;
  for ( CPI::OS::uint32_t n=0; n<getInputPortSetCount(); n++ ) {
    ps = static_cast<PortSet*>(tps[n]);
    for ( CPI::OS::uint32_t i=0; i<ps->getPortCount(); i++ ) {
...
namespace CPI {
  namespace DataTransport {
    ...
    private:
      volatile CPI::OS::uint8_t* current_src_buffer_ptr;
      volatile CPI::OS::uint64_t* current_src_metaData_ptr;
      volatile CPI::OS::uint32_t* current_empty_flag_ptr;
...
CPI::DataTransport::PullDataDriver* 
CPI::DataTransport::Circuit::
createPullDriver( CPI::RDT::Descriptors& pdesc, Port* p )
{
...
void 
CPI::DataTransport::Circuit::
setFlowControlDescriptor( CPI::DataTransport::Port* p, CPI::RDT::Descriptors& pdesc )
{

  // If the output port for this circuit is a shadow, now is a good time to 
  // make sure it gets initialized
  CPI::DataTransport::Port* sport = m_outputPs->getPort(0);
  ...
  //  4 of cpi but the port will not go ready until it is non zero
...
CPI::DataTransport::Circuit::
updatePort( CPI::DataTransport::Port* p )
{
...
getUserPortFlowControlDescriptor( CPI::RDT::Descriptors * fb, unsigned int idx )
{
  ...
  // completed.
  if ( getOutputPortSet()->getPort(0)->getMetaData()->m_descriptor.role == CPI::RDT::ActiveFlowControl ) {
    *fb = getOutputPortSet()->getPort(0)->getMetaData()->m_descriptor;
    fb->type = CPI::RDT::ProducerDescT;
  }
  else {
    *fb = sps->getPort(idx)->getMetaData()->m_shadowPortDescriptor;
    fb->type = CPI::RDT::ConsumerFlowControlDescT;
  }
...
CPI::DataTransport::Port* 
CPI::DataTransport::Circuit::
addInputPort( CPI::RDT::Descriptors& pdesc, const char* our_ep, CPI::RDT::Descriptors* fb )
{
  ...
  if ( m_metaData->m_portSetMd.getElementCount() <= 1 ) {
    ...
    PortSet * sps = static_cast<PortSet*>(m_inputPs[0]);
    Port* sport = 
      new CPI::DataTransport::Port( spsmd, static_cast<CPI::DataTransport::PortSet*>(this->getInputPortSet(0)) );
    sps->add( sport );
    ord++;
  }
  ...
  if ( pdesc.role == CPI::RDT::Passive ) {
    CPI::DataTransport::PortSet* portset = (CPI::DataTransport::PortSet*)this->getInputPortSet(0);
    CPI::DataTransport::Port* port = portset->getPort(0);
    printf("Found a Passive Consumer Port !!\n");
    PullDataDriver* pd = createPullDriver( pdesc, port );
    static_cast<CPI::DataTransport::Port*>(port)->attachPullDriver( pd );
  }
...
ready()
{
  ...
  CPI::DataTransport::PortSet* s_ps = getOutputPortSet();
  for (n=0; n<s_ps->getPortCount(); n++ ) {
    CPI::DataTransport::Port* sp = s_ps->getPort(n);
    if ( ! sp->ready() ) {
      return false;
    }
  }
...
bool 
CPI::DataTransport::Circuit::
updateConnection(CPI::DataTransport::Port*,CPI::OS::uint32_t)
{
  return ready();
}
...
// DEBUG CODE !!
  ...
  for ( CPI::OS::uint32_t n=0; n<this->m_connection->getInputPortSetCount(); n++ ) {
    CPI::DataTransport::PortSet* portset = (CPI::DataTransport::PortSet*)this->m_connection->getInputPortSet(n);
    for ( CPI::OS::uint32_t y=0; y<portset->getPortCount(); y++ ) {
  ...
  CPI::DataTransport::Port* sport = m_outputPs->getPort(0);
  for ( CPI::OS::uint32_t y=0; y<m_outputPs->getPortCount(); y++ ) {
    CPI::DataTransport::Port* port = m_outputPs->getPort(y);
    for ( CPI::OS::uint32_t b=0; b<port->getNumBuffers(); b++ ) {
...
createCircuitTemplateGenerators()
{
  ...
  CPI::DataTransport::PortSet *output_port_set = m_outputPs;
  ...
  for ( n=0; n<getInputPortSetCount(); n++ ) {
    ...
    // Input info
    CPI::DataTransport::PortSet *input_port_set = getInputPortSet(n);
    ...
    cpiAssert( output_port_set->getPort(0)->getMetaData()->m_descriptor.role < CPI::RDT::MaxRole );
    cpiAssert( input_port_set->getPort(0)->getMetaData()->m_descriptor.role < CPI::RDT::MaxRole );
...
checkIOZCopyQ()
{
  ...
  for ( CPI::OS::uint32_t n=0; n<m_maxPortOrd; n++ ) {
    ...
    CPI::OS::uint32_t n_queued = m_queuedInputOutputTransfers[n].getElementCount();
    total += n_queued;
    ...
    CPI::DataTransport::Buffer* input_buffer = 
                      ...
                      CPI::OS::uint32_t )
{
  ...
  cpiAssert( input_buf->m_zCopyPort == NULL );
               ...
               CPI::OS::uint32_t )
{
  ...
  cpiAssert( out_port->hasEmptyOutputBuffer() );
...
startBufferTransfer( Buffer* src_buf )
{
  ...
  for ( CPI::OS::uint32_t n=0; n<this->getQualifiedInputPortSetCount(); n++ ) {
    CPI::DataTransport::PortSet* t_port = this->getQualifiedInputPortSet(n);
...
broadcastBuffer( Buffer* src_buf )
{
  ...
  // We need to start a transfer for each input port set
  for ( CPI::OS::uint32_t n=0; n<this->getInputPortSetCount(); n++ ) {
    CPI::DataTransport::PortSet* t_port = this->getInputPortSet(n);
...
#ifdef QUE_CHECK
  // debug
  CPI::OS::int32_t n_queued = get_nentries(&m_queuedTransfers);
  Buffer* b1  = 
...
CPI::DataTransport::PortSet* 
CPI::DataTransport::Circuit::
getQualifiedInputPortSet(CPI::OS::uint32_t n, bool queued)
{
...
canTransferBuffer( Buffer* src_buf, bool queued_transfer )
{
  ...
  for ( CPI::OS::uint32_t n=0; n<this->getQualifiedInputPortSetCount(worst_case); n++ ) {
    CPI::DataTransport::PortSet *input_port_set = getQualifiedInputPortSet(n, worst_case);
    if ( ! input_port_set->getTxController()->canProduce( src_buf ) ) {
      return false;
    }
  }

  CPI_EMIT_( "Input Buffer Ready" );
...
debugDump()
{
  ...
  for ( n=0; n<getOutputPortSet()->getPortCount(); n++ ) {
    CPI::DataTransport::Port* port = static_cast<CPI::DataTransport::Port*>(this->getOutputPortSet()->getPortFromIndex(n));
    port->debugDump();
  }
  printf("  \nThere are %d input port sets\n", getInputPortSetCount() );
  for ( n=0; n<getInputPortSetCount(); n++ ) {
    printf("  Input port set %d, has %d ports\n",n,getInputPortSet(n)->getPortCount());
    for ( CPI::OS::uint32_t m=0; m<getInputPortSet(n)->getPortCount(); m++ ) {
      CPI::DataTransport::Port* port = static_cast<CPI::DataTransport::Port*>(this->getInputPortSet(n)->getPortFromIndex(m));
      port->debugDump();
    }
  }
}
