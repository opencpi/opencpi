...
using namespace OcpiCFUtilSCAFs;
...
overflow (int_type c)
{
  ocpiAssert (!pptr() || static_cast<unsigned int> (pptr() - pbase()) >= m_buffer->length());
  ...
  throw (std::string)
{
  ...
  return OCPI::Util::Vfs::joinNames (m_dir, fileName);
}
  ...
  throw (std::string)
{
  ...
  if (m_pbd.length()) {
    return OCPI::Util::Vfs::joinNames (m_pbd, fileName);
  }
  ...
  : m_cwd ("/")
{
  ...
  try {
    ...
    ocpiAssert (fsIorString.substr (0, 4) == "IOR:");
    ...
    throw OCPI::CORBAUtil::Misc::stringifyCorbaException (ex);
  }
}
  ...
  throw (std::string)
{
  ...
  return OCPI::Util::Uri::decode (uri.getPath ());
}
  ...
  throw (std::string)
{
  ...
  if (pattern.find ('/') != std::string::npos) {
    pbd = OCPI::Util::Vfs::directoryName (pattern);
  }

  std::string absPat = OCPI::Util::Vfs::joinNames (absName, pattern);
  std::string absDir = OCPI::Util::Vfs::directoryName (absPat);
