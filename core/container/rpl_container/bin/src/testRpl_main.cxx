...
int main(int argc, char *argv[])
{
  ...
  if (probe) {
    rplContainer = CC::Driver::getContainer("OCFRP", "0", "", 0);
  } else {
    CC::Driver::discoverContainers(0);
    rplContainer = CC::Driver::getContainer("OCFRP", firstarg, "", 0);
    if (two)
      rplContainer2 = CC::Driver::getContainer("OCFRP", secondarg, "", 0);
  }
  if (rplContainer && (!two || rplContainer2)) {
	...
	&a2 = two ? *ap2 : a;
      ...
      CC::PValue
	p00[] = {CC::PVULong("bufferCount", bufferCount[0][0]),
		 CC::PVString("xferRole", active[0][0]),
		 CC::PVULong("bufferSize", bufferSize), CC::PVEnd},
	p01[] = {CC::PVULong("bufferCount", bufferCount[0][1]),
		 CC::PVString("xferRole", active[0][1]),
		 CC::PVULong("bufferSize", bufferSize), CC::PVEnd},
	p10[] = {CC::PVULong("bufferCount", bufferCount[1][0]),
		 CC::PVString("xferRole", active[1][0]),
		 CC::PVULong("bufferSize", bufferSize), CC::PVEnd},
	p31[] = {CC::PVULong("bufferCount", bufferCount[3][1]),
		 CC::PVString("xferRole", active[3][1]),
		 CC::PVULong("bufferSize", bufferSize), CC::PVEnd},
	p60[] = {CC::PVULong("bufferCount", bufferCount[6][0]),
		 CC::PVString("xferRole", active[6][0]),
		 CC::PVULong("bufferSize", bufferSize), CC::PVEnd},
	p81[] = {CC::PVULong("bufferCount", bufferCount[8][1]),
		 CC::PVString("xferRole", active[8][1]),
		 CC::PVULong("bufferSize", bufferSize), CC::PVEnd};
	...
	w1in.loopback(w3out);
      if (acquire)
	w4out.connect(0, w1sin, 0);
      w1out.connect(0, w2in, 0);
      w2out.connect(0, w3in, 0);
      CC::UserPort *myOut =
	acquire ? 0 : &w1in.connectUser("w0out", p01, p10);
      if (two) {
	w3out.connect(p31, w6in, p60);
	w6out.connect(0, w7in, 0);
	w7out.connect(0, w8in, 0);
      }
      CC::UserPort &myIn =
	two ? w8out.connectUser("w0in", p00, p81) : w3out.connectUser("w0in", p00, p31);
      w[1]->start();
      w[2]->start();
      w[3]->start();
      if (two) {
	...
	uint32_t length;
	uint8_t *data;
	CC::UserBuffer *cBuffer;
	if (!acquire) {
	  CC::UserBuffer *pBuffer;
