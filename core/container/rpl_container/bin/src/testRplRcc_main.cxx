...
int main(int argc, char *argv[])
{
  ...
  CC::Worker * rcc_loopback;  
  CC::Artifact * rcc_art1;
  CC::Port     *rccPOut, *rccCIn, *rccLbOut, *rccLbIn;
  DThread * rcc_dThread;
  ...
  if ( use_rcc ) {
	...
	throw std::string("No RCC Containers found\n");
      ...
      rcc_container = static_cast<CPI::Container::Interface*>(d);
    ...
    static  const char * w1Url = "/home/jmiller/projects/opencpi/core/container/linux-dll/test.dll";
    try {
    ...
    catch ( CPI::Util::EmbeddedException & opps ) {
      printf("Got exception %s while trying to create the RCC Worker ports\n", opps.getAuxInfo() );
      throw;
    }
    catch ( ... ) {
  ...
  if (rplContainer && (!two || rplContainer2)) {
	...
	if ( use_rcc ) {
      ...
      catch ( CPI::Util::EmbeddedException & oops ) {
