  ...
  {
    {
      ...
      OA::Property p(app, APP_DEFAULT_XML_INST_NAME_PROXY, "rx_rf_gain");

      OA::Long actual_val = p.getLongValue();
...
bool did_pass_test_expected_value_proxy(
    OA::Application& app, const char* prop_name,
    OA::ULong expected_ad9361_config_proxy_val)
{
  try
  {
    {
      OA::Property p(app, APP_DEFAULT_XML_INST_NAME_PROXY, prop_name);

      OA::ULong actual_val = p.getULongValue();
      TEST_EXPECTED_VAL(actual_val, expected_ad9361_config_proxy_val);
    }
  }
  catch (std::string &e) {
    ...
    OA::ULong expected_ad9361_config_proxy_val)
{
  ...
  try
  {
    ...
    {
      OA::Property p(app, APP_DEFAULT_XML_INST_NAME_PROXY, "tx_sampling_freq");
