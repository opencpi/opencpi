...
\newpage
	...
	\textit{\textbf{Revision History}}
		...
		\label{table:revisions} % Add "[H]" to force placement of table
			...
			\hline
			v1.6 & Updated to include \textit{filerw\_rcc} mode & 8/2019 \\
			\hline
			\end{tabularx}
		\end{table}
	\end{center}
...
\section{References}
	...
	This document assumes a basic understanding of the Linux command line (or ``shell'') environment.  The references in Table 1 can be used as an overview of OpenCPI and may prove useful.
...
\hline
FSK App RCC Getting Started Guide\footnote{An RCC-only (software-only) implementation of \textit{this} guide} & \githubioFlat{assets/FSK\_App\_RCC\_Getting\_Started\_Guide.pdf} \\
}
...
\begin{flushleft}
\section{Overview}
This purpose of this document is to provide a compact set of instructions to build, run, and verify the OpenCPI Frequency Shift Keying Modulation (FSK) reference application (known as the ``FSK App'').
While the FSK reference application supports several execution modes (\textit{filerw}, \textit{filerw\_rcc}, \textit{txrx}, \textit{bbloopback}, \textit{rx}, \textit{tx}) and is supported on various systems, this guide will only discuss execution of the \textit{txrx} mode on the Epiq Solutions Matchstiq-Z1.
The \textit{filerw\_rcc} mode is introduced by the companion document \githubio[\textit{FSK App RCC Getting Started Guide}]{assets/FSK\_App\_RCC\_Getting\_Started\_Guide.pdf}.
...
\end{verbatim}
Reference the \textit{FSK\_app.pdf} for executing the various modes (\textit{filerw}, \textit{filerw\_rcc}, \textit{tx}, \textit{rx}, \textit{txrx}, \textit{bbloopback}) of the FSK application. Ensure that OCPI\_LIBRARY\_PATH is configured, as provided, for the respective mode and platform. \\ \medskip
