...
The input filename for the application is idata/Os.jpeg. It is modified from an original JPEG image (see Figure \ref{fig:os_pic}) with data prepended to it for baud synchronization purposes (240 bytes of an alternating 1-0 pattern followed by 2 bytes are the value 0xFACE). In the receive data stream, real\_digitizer.rcc worker makes symbol/bit decisions and only sends out bits that occur after the first detected 0xFACE bit pattern (b1111101011001110).
...
\noindent
\begin{itemize}
  \item The \textit{rx} and mode suffers from limited carrier recovery ability. When using \textit{rx} mode, the center frequency may need to be adjusted to a value other than the expected nominal value.
...
\subsection{Zedboard/FMCOMMS2/3}
	...
	\item target-xilinx13\_3/file\_read\_s.so
	\item target-xilinx13\_3/Baudtracking\_simple\_s.so
	\end{minipage}
	\begin{minipage}[t]{.5\textwidth}
	\item target-xilinx13\_3/real\_digitizer\_s.so
...
\\ \\
	\begin{minipage}[t]{.5\textwidth}
	\item target-xilinx13\_3/file\_read\_s.so
	\item target-xilinx13\_3/zipper\_tx\_s.so
	\end{minipage}
	\begin{minipage}[t]{.5\textwidth}
	\item target-xilinx13\_3/lime\_tx\_proxy\_s.so
	\item target-xilinx13\_3/si5351\_proxy\_s.so
	\end{minipage}
	\end{itemize}
...
\subsection{Matchstiq-Z1}
	...
	\begin{minipage}[t]{.5\textwidth}\item target-xilinx13\_3/Baudtracking\_simple\_s.so
	\item target-xilinx13\_3/real\_digitizer\_s.so
	\item target-xilinx13\_3/file\_write\_s.so
	\item target-xilinx13\_3/matchstiq\_z1\_rx\_s.so
	\item target-xilinx13\_3/lime\_rx\_proxy\_s.so
	\end{minipage}
	\begin{minipage}[t]{.5\textwidth}	\item target-xilinx13\_3/si5338\_proxy\_s.so
...
mfsk2\_zp16\_fir\_real\_phase\_to\_amp\_cordic\_cic\_int\_matchstiq\_z1\_matchstiq\_z1\_tx\_cnt\_0rx\_1tx\_thruasm\_matchstiq\_z1.bitz	\\ \\
	\begin{minipage}[t]{.5\textwidth}
	\item target-xilinx13\_3/file\_read\_s.so
	\item target-xilinx13\_3/matchstiq\_z1\_tx\_s.so
	\item target-xilinx13\_3/lime\_tx\_proxy\_s.so
...
\subsection{Stratix IV/Zipper}
	...
	\item target-centos7/file\_read\_s.so
	\item target-centos7/Baudtracking\_simple\_s.so
	\end{minipage}
	\begin{minipage}[t]{.5\textwidth}
	\item target-centos7/real\_digitizer\_s.so
	...
	\begin{minipage}[t]{.5\textwidth}
	\item target-centos7/file\_read\_s.so
	\item target-centos7/zipper\_tx\_s.so
	\end{minipage}
	\begin{minipage}[t]{.5\textwidth}
	\item target-centos7/lime\_tx\_proxy\_s.so
	\item target-centos7/si5351\_proxy\_s.so
	\end{minipage}
	\end{itemize}
	For Zipper plugged into HSMC Port A:
