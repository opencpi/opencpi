 ...
 *
 * This file contains the implementation skeleton for the fir_real_sse_for_fskapp worker in C++
 */
...
class Fir_real_sse_for_fskappWorker : public Fir_real_sse_for_fskappWorkerBase {
     ...
     ***/
    RCCResult release(){
        firfilt_crcf_destroy(m_firfilt_handler);
        return RCC_OK;
    }
    ...
    void perform_filter_operation(const size_t num_of_elements) {
        ...
        liquid_float_complex sample_in;  //liquid dsp sample data type
        liquid_float_complex sample_out;  //liquid dsp sample data type

        for (size_t i = 0; num_of_elements > i; i++){
            sample_in.real = uscale(m_inData_ptr);

            firfilt_crcf_push(m_firfilt_handler, sample_in);
            firfilt_crcf_execute(m_firfilt_handler, &sample_out);

            *m_outData_ptr++ = perform_truncation(sample_out);
            m_inData_ptr++;
    	}
    }
