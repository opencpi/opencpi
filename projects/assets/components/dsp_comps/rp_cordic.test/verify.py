...
num_samples = int(sys.argv[1]) - 8 - int(os.environ.get("OCPI_TEST_STAGES"))
# The measured peak magnitude from the UUT
magnitude   = int(os.environ.get("OCPI_TEST_magnitude")) - 1
...
for i in xrange(0,num_samples):
    ...
    phase_exp[i]     = 32768 / np.pi * math.atan2( imag[i], real[i] )
...
if (check_AvgPeakError > 1) or (mag_peak_err > 1) or (pha_peak_err > 1):
