...
\begin{flushleft}
The {\comp} component takes input port messages and stores their data in a buffer via the \texttt{data} property as 4 byte words. Metadata associated with each message is stored as a record in a metadata buffer exposed by the \texttt{metadata} property. Each record contains four 4 byte words of metadata. The first word contains the opcode for the message and message size in bytes; opcode 8 MSB and message size 24 LSB. The second word contains the unsigned Q0.32 fractional portion of the timestamp for the End-Of-Message (EOM). The third word contains the fractional portion of the timestamp for the Start-Of-Message (SOM). The fourth word contains the unsigned Q32.0 integer portion of the timestamp for the SOM. All timestamps are provided by the worker's time service interface. So that the metadata can be read on a little-endian processor, the ordering of the metadata is as follows: \newline
