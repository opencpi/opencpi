...
\noindent\begin{scriptsize}
	\begin{longtable}{|M{4.4cm}|M{1.9cm}|M{1.9cm}|M{1.5cm}|M{1.8cm}|M{1.7cm}|M{1.4cm}|}
		...
		\hline
		\rowcolor{blue}
    Device                                 & Registers (typical) & LUTS (typical) & \multicolumn{2}{c|}{Worker Fmax (typical)}  & Memory/ Special Functions & Design Suite \\
		\hline
    \rowcolor{blue}
...
\footnotetext[2]{\label{notfullrange}Note that a) this condition is driven by the limitations of the capabilities of Xilinx constraints, and b) the range of value(s) for this condition does not cover the full range of possible values for AD9361 for the given mode.}
\pagebreak
\begin{scriptsize}
	\begin{longtable}{|c|l|c|c|c|c|l|}
...
\begin{scriptsize}
    ...
    Parameter & \multicolumn{1}{|c|}{Conditions} & Min & Typ & Max & Unit & \multicolumn{1}{|c|}{Notes} \\
		\hline
    \multirow{42}{*}{\parbox{1.7cm}{RX bit error rate}} & \parbox{4.5cm}{Zedboard w/ FMC voltage jumper \\ setting 2.5V, FMCOMMS2/3} & & & & & \multirow{42}{*}{AD9361 PRBS test} \\
                          &                                      &                                & &                                &                      & \\
