...
echo "usage: ./test_hsmc_debug_header_breakout_is_plugged_into_port_b.sh    #this will run the whole script without prompts"
echo "usage: ./test_hsmc_debug_header_breakout_is_plugged_into_port_b.sh -p # this will prompt you for each LED"

echo "searching for OpenCPI-enabled PCIE devices..."
PCIE_DEV_NUM=$(ocpihdl search | sed "s/OpenCPI HDL device found: 'PCI:0000://g" | sed "s/:00.0.*//g")
HSMC_LOOPBACK_WORKER_INSTANCE_NUM=c/HSMC_ALST4_B_hsmc_debug_header_breakout

echo "loading and initializing FPGA bitfile..."

ocpihdl -d $PCIE_DEV_NUM load assemblies/hsmc_debug_header_breakout_test_assembly/container-hsmc_debug_header_breakout_test_assembly_alst4_base_hsmc_debug_header_breakout_card_plugged_into_hsmc_port_b_container/target-stratix4/hsmc_debug_header_breakout_test_assembly_alst4_base_hsmc_debug_header_breakout_card_plugged_into_hsmc_port_b_container.sof > /dev/null 2>&1
