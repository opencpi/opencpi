...
#include "ad9361_common.h" // regs_general_rfpll_divider_t
...
const char* calc_AD9361_Tx_RFPLL_LO_freq_Hz(
  ...
  { // restrict scope so we don't accidentally use non-double values
    ...
    {
      bool& val = Tx_RFPLL_external_div_2_enable;
      const char* ret = calc_AD9361_Tx_RFPLL_external_div_2_enable(val, regs);
      if(ret != 0) {
        return ret;
      }
    }
    ...
    {
      float& val = Tx_RFPLL_ref_divider;
      const char* ret = calc_AD9361_Tx_RFPLL_ref_divider(val, regs);
      if(ret != 0) {
        return ret;
      }
    }
    ...
    {
      uint16_t& val = Tx_RFPLL_N_Integer;
      const char* ret = calc_AD9361_Tx_RFPLL_N_Integer(val, regs);
      if(ret != 0) {
        return ret;
      }
    }
    ...
    {
      uint32_t& val = Tx_RFPLL_N_Fractional;
      const char* ret = calc_AD9361_Tx_RFPLL_N_Fractional(val, regs);
      if(ret != 0) {
        return ret;
      }
    }
    ...
    {
      uint8_t& val = Tx_RFPLL_VCO_Divider;
      const char* ret = calc_AD9361_Tx_RFPLL_VCO_Divider(val, regs);
      if(ret != 0) {
        return ret;
      }
    }
