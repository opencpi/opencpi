...
#include "RadioCtrlrConfiguratorFMCOMMS2TuneResampTester.hh" // ConfiguratorFMCOMMS2TuneResampTester

namespace OCPIProjects {

namespace RadioCtrlr {

ConfiguratorFMCOMMS2TuneResampTester::ConfiguratorFMCOMMS2TuneResampTester(
    ConfiguratorFMCOMMS2TuneResamp<>& UUT) : ConfiguratorTester(UUT) {
}

bool ConfiguratorFMCOMMS2TuneResampTester::run_tests() {
  try {
...
void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_RX_gain_mode_abs_range() {
...
void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_TX_gain_mode_abs_range() {
  std::vector<data_stream_ID_t> data_stream_keys;
...
void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_RX_gain_abs_range() {
  std::vector<data_stream_ID_t> data_stream_keys;
  data_stream_keys.push_back("SMA_RX1A");
  data_stream_keys.push_back("SMA_RX2A");
  for(auto it=data_stream_keys.begin(); it!=data_stream_keys.end(); it++) {
...
void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_TX_gain_abs_range() {
  std::vector<data_stream_ID_t> data_stream_keys;
  data_stream_keys.push_back("SMA_TX1A");
  data_stream_keys.push_back("SMA_TX2A");
  for(auto it=data_stream_keys.begin(); it!=data_stream_keys.end(); it++) {
...
void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_tuning_freq_equals_RFPLL_LO_freq_plus_complex_mixer_NCO_freq(data_stream_ID_t data_stream_ID, config_key_t RFPLL_LO_freq) {
  config_value_t expected_min_NCO_freq = -30.72; // 61.44*-32768./65536.;
  config_value_t expected_max_NCO_freq = 61.44*32767./65536.;
  config_value_t& min = expected_min_NCO_freq;
  config_value_t& max = expected_max_NCO_freq;
...
void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_data_stream_tuning_freq_by_RFPLL_LO_freq_plus_data_stream_complex_mixer_NCO_freq(
  ...
  m_UUT.unlock_all();
  test_config_lock(RFPLL_LO_freq,                                   2401.,     true );
  m_UUT.unlock_all();
  test_config_lock(RFPLL_LO_freq,                                   2450.,     true );
  m_UUT.unlock_all();
  test_config_lock(RFPLL_LO_freq,                                   2499.,     true );
  m_UUT.unlock_all();
  ...
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2401.,     true );
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2450.,     true );
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2499.,     true );
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", min,       true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2401.+min, false);
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", min,       true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2401.,     true );
  m_UUT.unlock_all();
  ...
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", min,       true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2499.,     false);
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", min,       true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2499.+min, true );
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", 0.,        true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2401.+min, false);
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", 0.,        true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2451.,     true );
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", 0.,        true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2499.,     true , 0.0000001);
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", max,       true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2401.+max, true );
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", max,       true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2401.,     false);
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", max,       true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2450.,     true );
  m_UUT.unlock_all();
  ...
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", max,       true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2499.+max, false, 0.0000001);
  m_UUT.unlock_all();
  test_config_lock(RFPLL_LO_freq,                                   2401.,     true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2401.+min, false);
  m_UUT.unlock_all();
  ...
  m_UUT.unlock_all();
  test_config_lock(RFPLL_LO_freq,                                   2450.,     true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2450.+min, true );
  m_UUT.unlock_all();
  test_config_lock(RFPLL_LO_freq,                                   2450.,     true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2450.,     true );
  m_UUT.unlock_all();
  test_config_lock(RFPLL_LO_freq,                                   2450.,     true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2450.+max, true,  0.0000001);
  m_UUT.unlock_all();
  test_config_lock(RFPLL_LO_freq,                                   2499.,     true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2499.+min, true );
  m_UUT.unlock_all();
  ...
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2499.,     true );
  m_UUT.unlock_all();
  test_config_lock(RFPLL_LO_freq,                                   2499.,     true , 0.0000001);
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz,     2499.+max, false, 0.0000002);
}

void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_RFPLL_LO_freq_by_data_stream_tuning_freq_minus_data_stream_complex_mixer_NCO_freq(
    data_stream_ID_t data_stream_ID,
    config_key_t RFPLL_LO_freq,
    config_value_t expected_min_NCO_freq,
    config_value_t expected_max_NCO_freq) {
  ...
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", min      , true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz    , 2401.    , true );
  test_config_lock(RFPLL_LO_freq                                  , 2450.    , false);
  test_config_lock(RFPLL_LO_freq                                  , 2499.    , false);
  test_config_lock(RFPLL_LO_freq                                  , 2401.-min, true );
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", min      , true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz    , 2450.    , true );
  test_config_lock(RFPLL_LO_freq                                  , 2401.    , false);
  test_config_lock(RFPLL_LO_freq                                  , 2499.    , false);
  test_config_lock(RFPLL_LO_freq                                  , 2450.-min, true );
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", min      , true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz    , 2499.    , false);
  ...
  test_config_lock(RFPLL_LO_freq                                  , 2450.    , false);
  test_config_lock(RFPLL_LO_freq                                  , 2499.    , false);
  test_config_lock(RFPLL_LO_freq                                  , 2401.-0. , true );
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", 0.       , true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz    , 2450.    , true );
  test_config_lock(RFPLL_LO_freq                                  , 2401.    , false);
  test_config_lock(RFPLL_LO_freq                                  , 2499.    , false);
  test_config_lock(RFPLL_LO_freq                                  , 2450.-0. , true );
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", 0.       , true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz    , 2499.    , true );
  test_config_lock(RFPLL_LO_freq                                  , 2401.    , false);
  test_config_lock(RFPLL_LO_freq                                  , 2450.    , false);
  test_config_lock(RFPLL_LO_freq                                  , 2499.-0. , true , 0.0000001);
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", max      , true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz    , 2401.    , false);
  ...
  test_config_lock(RFPLL_LO_freq                                  , 2401.    , false);
  test_config_lock(RFPLL_LO_freq                                  , 2499.    , false);
  test_config_lock(RFPLL_LO_freq                                  , 2450.-max, true );
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", max      , true );
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz    , 2499.    , true );
  test_config_lock(RFPLL_LO_freq                                  , 2401.    , false);
  test_config_lock(RFPLL_LO_freq                                  , 2450.    , false);
  test_config_lock(RFPLL_LO_freq                                  , 2499.-max, true , 0.0000001);
}

void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_data_stream_complex_mixer_NCO_freq_by_RFPLL_LO_freq_minus_data_stream_tuning_freq(
    data_stream_ID_t data_stream_ID,
    config_key_t RFPLL_LO_freq,
    config_value_t expected_min_NCO_freq,
    config_value_t expected_max_NCO_freq) {
  ...
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", 0.       , false);
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", max      , false);
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", min      , true , 0.0000002);
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz    , 2401.+0. , true );
  test_config_lock(RFPLL_LO_freq                                  , 2401.    , true );
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", min      , false);
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", max      , false);
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", 0.       , true );
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz    , 2450.+0. , true );
  test_config_lock(RFPLL_LO_freq                                  , 2450.    , true );
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", min      , false);
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", max      , false);
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", 0.       , true );
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz    , 2499.+0. , true );
  test_config_lock(RFPLL_LO_freq                                  , 2499.    , true );
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", min      , false);
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", max      , false);
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", 0.       , true );
  ...
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", max      , true , 0.0000001);
  m_UUT.unlock_all();
  test_config_lock(data_stream_ID, config_key_tuning_freq_MHz    , 2450.+max, true );
  test_config_lock(RFPLL_LO_freq                                  , 2450.    , true );
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", min      , false);
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", 0.       , false);
  test_config_lock(data_stream_ID, "tuning_freq_complex_mixer_MHz", max      , true ,0.0000001);
...
void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_RX_gain_limits() {
  std::vector<data_stream_ID_t> data_stream_keys;
  data_stream_keys.push_back("SMA_RX1A");
  data_stream_keys.push_back("SMA_RX2A");
  for(auto it=data_stream_keys.begin(); it!=data_stream_keys.end(); it++) {
    ...
    m_UUT.unlock_all();
    test_config_lock("Rx_RFPLL_LO_freq",               2401., true );
    test_config_lock(*it, config_key_gain_dB,         -10.,  false);
    m_UUT.unlock_all();
    test_config_lock("Rx_RFPLL_LO_freq",               2401., true );
    test_config_lock(*it, config_key_gain_dB,         -3.,   true );
...
void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_DS_bandwidth_equals_FE_bandwidth_divided_by_CIC_dec(
    data_stream_ID_t data_stream_ID,
...
void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_DS_bandwidth_equals_FE_bandwidth_divided_by_CIC_int(
    data_stream_ID_t data_stream_ID,
    config_key_t frontend_bandwidth) {
...
void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_RX_SAMPL_FREQ_to_TX_SAMPL_FREQ_multiplied_by_DAC_Clk_divider() {
...
void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_TX_SAMPL_FREQ_to_RX_SAMPL_FREQ_divided_by_DAC_Clk_divider() {
  test_constrain_RX_SAMPL_FREQ_to_TX_SAMPL_FREQ_multiplied_by_DAC_Clk_divider();
}

void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_DAC_Clk_divider_to_RX_SAMPL_FREQ_divided_by_TX_SAMPL_FREQ() {
  m_UUT.unlock_all();
  test_config_lock("DAC_Clk_divider", 1., true );;
  m_UUT.unlock_all();
  test_config_lock("DAC_Clk_divider", 2., true );;
...
void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_sampling_rate_equals_FE_samp_rate_divided_by_CIC_dec(
    data_stream_ID_t data_stream_ID,
...
void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_sampling_rate_equals_FE_samp_rate_divided_by_CIC_int(
    data_stream_ID_t data_stream_ID,
    config_key_t frontend_samp_rate) {
  config_value_t min = 4; // assumed min CIC_int interpolation factor
  config_value_t max = 8192; // assumed max CIC_int interpolation factor
...
void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_FE_samp_rate_to_func_of_DS_complex_mixer_freq(
    data_stream_ID_t data_stream_ID,
    config_key_t frontend_samp_rate,
    config_value_t expected_min_NCO_freq,
    config_value_t expected_max_NCO_freq,
...
void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_DS_complex_mixer_freq_to_func_of_FE_samp_rate(
    data_stream_ID_t data_stream_ID,
    config_key_t   frontend_samp_rate,
    config_value_t expected_min_NCO_freq,
    config_value_t expected_max_NCO_freq,
...
void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_FE_samp_rate_equals_func_of_DS_complex_mixer_freq(
    data_stream_ID_t data_stream_ID,
    config_key_t frontend_samp_rate,
    config_value_t expected_min_NCO_freq,
    config_value_t expected_max_NCO_freq,
 ...
 *                         sampling rate.
 ****************************************************************************/
void ConfiguratorFMCOMMS2TuneResampTester::test_constrain_Nyquist_criteria(
