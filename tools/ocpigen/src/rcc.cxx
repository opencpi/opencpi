...
emitRccArgTypes(FILE *f, bool &first) {
  if (operations()) {
	...
	if (other.operations() &&
	    !strcasecmp(other.OU::Protocol::m_name.c_str(), OU::Protocol::m_name.c_str()))
	  return;
      }
    if (first)
      fprintf(f,
    ...
    std::string prefix;
    if (nOperations()) {
      camel(prefix, OU::Protocol::name().c_str());
      fprintf(f,
	      "  // Enumeration constants for the operations of protocol \"%s\"\n"
	      "  enum %sOpCodes { \n",
	      OU::Protocol::name().c_str(), prefix.c_str());
	...
	if (m->m_baseType == OA::OCPI_Enum || m->m_baseType == OA::OCPI_Struct) {
	  if (pfirst)
	    fprintf(f,
		    ...
		    "   */\n",
		    OU::Protocol::name().c_str());
	  if (ofirst)
	    fprintf(f,
		    "  /* Argument data types for the \"%s\" operation of the \"%s\" protocol */\n",
		    o->name().c_str(), OU::Protocol::name().c_str());
	  pfirst = ofirst = false;
	  camel(prefix, OU::Protocol::name().c_str(), o->name().c_str(), NULL);
	  ...
	  for (unsigned l = worker().m_maxLevel+1; l--; ) {
	    std::string type;
	    worker().rccType(type, *m, 0, offset, pad, prefix.c_str(), true, isLastDummy,
			      nn == 0 && o->nArgs() == 1, l);
	    fputs(type.c_str(), f);
	  }
	}
...
emitRccCppImpl(FILE *f) {
  ...
  if (o) {
    ...
    for (unsigned nn = 0; nn < nOperations(); nn++, o++) {
      ...
      if (o->nArgs()) {
	std::string op;
	camel(op, worker().m_implName, "WorkerTypes::", OU::Protocol::name().c_str(),
	      o->name().c_str());
	fprintf(f,
	...
	for (unsigned n = 0; n < o->nArgs(); n++, m++) {
	  ...
	  camel(p, cname() );	
	  std::string on;
	  camel(on, worker().m_implName, "WorkerTypes::", OU::Protocol::name().c_str(),
		o->name().c_str());
	  std::string type;
	  size_t offset;
	  unsigned pad;
	...
	camel(s, o->name().c_str());
	camel(prot, OU::Protocol::m_name.c_str());
	camel(prefix, worker().m_implName, "WorkerTypes::", prot.c_str(),
	      o->name().c_str());
	if (first) {
	  fprintf(f, "  public:\n");
	  first = false;
...
emitRccCImpl1(FILE *f) {
  if (operations()) {
	...
	fprintf(f,
		...
		o->name().c_str(), cname());
	std::string s;
	camel(s, worker().m_implName, cname(), o->name().c_str());
	bool isLast = false;
	std::string type;
	  ...
	  for (unsigned a = 0; a < o->m_nArgs; a++, arg++) {
	    ...
	    if (ap && ap->m_scaling.m_min) {
	      fprintf(f, "  %sPartInfo %s_info[%zu];\n",
		      upperdup(worker().m_modelString), arg->m_name.c_str(),
...
RccPort(Worker &w, ezxml_t x, DataPort *sp, int ordinal, const char *&err)
  ...
  if (x && !err &&
      !(err = OE::checkAttrs(x, SPEC_DATA_PORT_ATTRS, "implname", "minbuffers", "minbuffercount",
			     "buffersize", "buffercount", "internal", DISTRIBUTION_ATTRS,
			     PARTITION_ATTRS, (void*)0)))
