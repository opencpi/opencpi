...
HdlContainer(HdlConfig &config, HdlAssembly &appAssembly, ezxml_t xml, const char *xfile,
  ...
  if (!platform)
    platform = m_config.platform().name();
  ...
  for (PortsIter pi = m_config.m_ports.begin(); pi != m_config.m_ports.end(); pi++) {
	...
	Port &sp = **si;
	if (!sp.master && sp.type == NOCPort && !strncasecmp(p.cname(), sp.cname(), len) &&
	    !strcasecmp(sp.cname() + len, "_slave")) {
	  slave = &sp;
	  break;
	}
      }
  ...
  if (doDefault) {
	...
	OU::formatAdd(assy,
		      ...
		      "  </connection>\n",
		      nWCIs, (*di).name(),
		      (*di).device.deviceType().ports()[0]->cname());
	...
	if ((*pi)->type == WTIPort)
	  emitTimeClient(assy, (*di).name(), (*pi)->cname());
    }
...
emitConnection(std::string &assy, UNocs &uNocs, size_t &index, const ContConnect &c) {
  ...
  if (c.interconnect) {
    ...
    std::string devport;
    if (c.devInConfig)
      OU::format(devport, "%s_%s", c.devInstance->name(), c.port->cname());
    OU::formatAdd(assy,
		  "  <connection>\n"
		  "    <port instance='%s' name='%s'/>\n"
		  "    <port instance='%s' name='%s'/>\n"
