...
const char *checkSuffix(const char *str, const char *suff, const char *last) {
  size_t nstr = (size_t)(last - str), nsuff = strlen(suff);
...
findPackage(ezxml_t spec, const char *a_package) {
  ...
  if (a_package)
    ...
    if (cp)
      packageFileDir.assign(base, (size_t)(cp + 1 - base));
    ...
    for (cp = m_package.c_str(); *cp && isspace(*cp); cp++)
      ;
    m_package.erase(0, (size_t)(cp - m_package.c_str()));
    for (cp = m_package.c_str(); *cp && !isspace(*cp); cp++)
      ;
    m_package.resize((size_t)(cp - m_package.c_str()));
  }
  return NULL;
}
...
getNames(ezxml_t xml, const char *file, const char *tag, std::string &name,
  ...
  if (file) {
    ...
    fileName.assign(cp, lp ? (size_t)(lp - cp) : strlen(cp));
  }
  if (fileName.empty())
...
addLibMap(const char *map) {
  ...
  const char *newLib = cp ? strndup(map, (size_t)(cp - map)) : strdup(map);
  if (cp)
    cp++;
