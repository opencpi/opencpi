...
printSimple(FILE *f, Simple *t, const char *prefix, unsigned &offset, unsigned &pad)
{
  ...
  fprintf(f, "%s  %-12s %s", prefix, rccTypes[t->type], t->name);
  if (t->type == CM::Property::CPI_String)
    fprintf(f, "[%lu]", roundup(t->stringLength + 1, 4));
...
static void
emitStructRCC(FILE *f, unsigned nTypes, Simple *types, const char *indent) {
  Simple *t = types;
  unsigned align = 0, pad = 0;
  for (unsigned n = 0; n < nTypes; n++, t++)
...
emitImplRCC(Worker *w, const char *outDir, const char *library) {
  ...
  if (w->nPorts) {
    ...
    for (unsigned n = 0; n < w->nPorts; n++, p++) {
      fprintf(f, "%s  %s_%s", last, upper, upperdup(p->name));
      // FIXME TWO WAY
      last = ",\n";
      if (p->wdi.isProducer)
	out++;
	...
	if (p->isStruct) {
	  ...
	  emitStructRCC(f, p->nTypes, p->types, "  ");
	  fprintf(f, "  } %s", p->name);
	  if (p->isStructSequence)
	    fprintf(f, "[%u]", p->nStructs);
	  fprintf(f, ";\n");
	} else
	  printSimple(f, p->types, "", align, pad);
      }
      fprintf(f,
	      "} %c%sProperties;\n\n",
	      toupper(w->implName[0]), w->implName + 1);
	...
	if (p->wdi.nOperations) {
	  fprintf(f,
		  ...
		  "typedef enum {\n",
		  p->name, w->implName);
	  Operation *o = p->wdi.operations;
	  const char *puName = upperdup(p->name);
	  for (unsigned nn = 0; nn < p->wdi.nOperations; nn++, o++)
