...
#
# - Device assignments are solely based on the CPIContainerType, CPIDeviceId,
#   CPIDeviceType, os_name and processor_name attributes.
...
itcl::class cfutil::ApplicationFactory {
    ...
    public common s_cpiContainerTypeUUID "DCE:c4b738d8-fbe6-4893-81cd-1bb7a77bfb43"
    public common s_cpiDeviceIdUUID "DCE:b59fa5e6-5eb4-44f6-90f6-0548508f2ba2"
    public common s_cpiDeviceTypeUUID "DCE:c788404e-b9f5-4532-8c7d-3588d328fff0"
	...
	foreach implementation [getAllChildren $spd "implementation"] {
		...
		#
		# Ensure that a non-CPI component is not assigned to a CPI
		# device, except to a GPP device.
		#

		if {$isCompatible && \
			![info exists requiredCapacity($s_cpiContainerTypeUUID)] && \
			[info exists deviceCapacity($s_cpiContainerTypeUUID)] && \
			$deviceCapacity($s_cpiContainerTypeUUID) != "GPP"} {
		    if {$m_verbose > 3} {
			puts "      CPI device \"$deviceCapacity(label)\" is not compatible with non-CPI component."
		    }
	...
	# non-CPI component to any non-CPI executable device, just in
	# chance.
	#
	# The problem is that a non-CPI component still advertises its
	# os and processor dependencies, but non-CPI executable devices
	...
	if {![llength $compatibleDevices]} {
	    ...
	    foreach implementation [getAllChildren $spd "implementation"] {
		set implId [$implementation getAttribute "id"]
		set isCpiComponent 0
		puts "Checking implementation $implId again."

		foreach dependency [getAllChildren $implementation "dependency"] {
		    foreach propertyref [getAllChildren $dependency "propertyref"] {
			set refid [$propertyref getAttribute "refid"]
			if {$refid eq $s_cpiContainerTypeUUID} {
			    set isCpiComponent 1
			    break
			}
		    }
		    if {$isCpiComponent} {
		...
		puts "Checking implementation $implId again: $isCpiComponent."
		...
		if {!$isCpiComponent} {
			...
			if {[info exists deviceCapacity($s_cpiContainerTypeUUID)]} {
			    puts "Device $deviceCapacity(label) is a CPI device."
			    set isCpiDevice 1
			} else {
			    set isCpiDevice 0
			}
			...
			if {!$isCpiDevice} {
			    if {$m_verbose > 3} {
				puts "      Non-CPI implementation \"$implId\" assumed compatible with non-CPI device \"$deviceCapacity(label)\""
			    }
	...
	foreach ed $eds {
		...
		if {$m_verbose} {
		    ...
		    if {[info exists edInfo($s_cpiContainerTypeUUID)]} {
			switch -- $edInfo($s_cpiContainerTypeUUID) {
			    GPP {
				puts -nonewline "Found GPP Executable Device \"$edInfo(label)\", "
				puts -nonewline "device id \"$edInfo($s_cpiDeviceIdUUID)\" "
				...
				puts -nonewline "device id \"$edInfo($s_cpiDeviceIdUUID)\", "
				puts "($edInfo($s_cpiDeviceTypeUUID))."
			    }
			}
		    } else {
	...
	#
	# Query the Executable Device's CPIContainerType property.
	#
	...
	set cpiContainerType [lindex [lindex [lindex $props 0] 3] 1]
	lappend info $s_cpiContainerTypeUUID $cpiContainerType

	switch -- $cpiContainerType {
	    "GPP" {
		set ids [list $s_cpiDeviceIdUUID $s_osNameUUID $s_processorNameUUID]
	    }
	    "RCC" {
		set ids [list $s_cpiDeviceIdUUID $s_osNameUUID $s_processorNameUUID]
	    }
	    "RPL" {
		set ids [list $s_cpiDeviceIdUUID $s_cpiDeviceTypeUUID]
	    }
	    default {
		#
		...
		error "invalid container type $cpiContainerType"
	    }
	}
