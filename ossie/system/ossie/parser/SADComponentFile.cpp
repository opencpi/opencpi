...
SADComponentFile::SADComponentFile(TiXmlElement *elem)
{
...
void SADComponentFile::parseElement(TiXmlElement *elem)
{
    type = elem->Attribute("type");
    id = elem->Attribute("id");

    TiXmlElement *local = elem->FirstChildElement("localfile");
    fileName = local->Attribute("name");

    DEBUG(7, SADComponentFile, "Found type: " << type << ", id: " << id << ", fileName: " << fileName);
}
