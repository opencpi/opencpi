...
public:
    complexFloat_u(const char* portName);
    ~complexFloat_u();
...
private:
    complexFloat_u();
    complexFloat_u(const complexFloat_u &);
...
{
public:
    usesPort(standardInterfaces_i::complexFloat_u *_base);
    ~usesPort();
...
{
public:
    ConnectionInfo(standardInterfaces::complexFloat_ptr _port, const char* _ID) : port_obj( _port), identifier(_ID) { };
    ConnectionInfo(const ConnectionInfo & c) {
        port_obj = c.port_obj;
        identifier = c.identifier;
    };

    void setPort(standardInterfaces::complexFloat_ptr _port) {
        port_obj = _port;
    };
    const char *getID() {
