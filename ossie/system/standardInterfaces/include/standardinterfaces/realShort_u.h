...
public:
    realShort_u(const char* portName);
    realShort_u(const char* portName, const char* domainName);
    ~realShort_u();
...
{
public:
    usesPort(standardInterfaces_i::realShort_u *_base);
    ~usesPort();
...
{
public:
    ConnectionInfo(standardInterfaces::realShort_ptr _port, const char* _ID) : port_obj( _port), identifier(_ID) { };
    ConnectionInfo(const ConnectionInfo & c) {
        port_obj = c.port_obj;
        identifier = c.identifier;
    };

    void setPort(standardInterfaces::realShort_ptr _port) {
        port_obj = _port;
    };
    const char *getID() {
