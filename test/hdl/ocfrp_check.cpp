...
#define PCIDIR "/sys/bus/pci/devices"
	...
	if (pciMemFd < 0 && (pciMemFd = open("/dev/mem", O_RDWR)) < 0)
	  ...
	  if (occp->admin.magic1 != OCCP_MAGIC1 || occp->admin.magic2 != OCCP_MAGIC2) {
	    ...
	    printf("Found an OpenCPI reference platform with bitstream birthday: %s", ctime_r(&bd, tbuf));
	    foundIt = true;
	  }
	}
	if (err) {
...
{
  const char *err = CPI::RPL::search(0, OCFRP0_VENDOR, OCFRP0_DEVICE,
				     OCFRP0_CLASS, OCFRP0_SUBCLASS, CPI::RPL::found,
				     argv[1] && strcmp(argv[1], "-v") == 0);
  if (err) {
    fprintf(stderr, "PCI Scanner Error: %s\n", err);
    return 1;
  } else if (!CPI::RPL::foundIt) {
    fprintf(stderr, "Did not find an OpenCPI FPGA reference platform.\n");
    return 1;
  }
  return 0;
