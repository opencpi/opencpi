  ...
  while (!err && (ent = readdir(pcid)) != NULL) {
	...
	for (unsigned i = 0; !err && i < 6; i++) {
	  ...
	  if (bottom && bottom != top) {
	    ...
	    if (verbose)
	      printf("   BAR %d: 0x%llux to 0x%llux (%llu%s %db %s %s)\n",
		     i, bottom, top, 
		     (unsigned long long)
...
static void
found(Bar *bars, unsigned nbars, bool verbose) {
  const char *err = 0;
  if (nbars != 2 || bars[0].io || bars[0].prefetch || bars[1].io || bars[1].prefetch ||
	...
	printf("Found an OpenOCPI reference platform with bitstream birthday: %s",
	       ctime_r(&bd, tbuf));
	foundIt = true;
      }
    }
...
{
  const char *err = search(0, OCFRP0_VENDOR, OCFRP0_DEVICE,
			   OCFRP0_CLASS, OCFRP0_SUBCLASS, &found,
