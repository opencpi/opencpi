...
\begin{center}
  ...
  \begin{table}[H]
    \begin{tabularx}{\textwidth}{|c|X|l|}
      ...
      \hline
      v1.2 & Initial creation for ANGRYVIPER 1.2 & 8/2017 \\
      \hline
    \end{tabularx}
  \end{table}
...
\begin{flushleft}
This document explains usage of Xilinx Vivado in the context of ANGRYVIPER. For further information regarding Xilinx Vivado, consult Xilinx's documentation (\textit{e.g.} \href{https://www.xilinx.com/support/documentation/sw_manuals/xilinx2017_1/ug835-vivado-tcl-commands.pdf}{UG835}).
...
This process is described in \code{ocpiassets/hdl/platforms/matchstiq\_z1/ise\_constraints/README} for the ``matchstiq\_z1'' platform. To summarize more generically:
\begin{enumerate}
...
\section{Using PreBuilt Cores/Netlists with Vivado and ANGRYVIPER}
\label{prebuiltcore}
...
Another option for including prebuilt cores with ANGRYVIPER is to create an ANGRYVIPER primitive core. In the primitive core's makefile, you would set ``\code{PreBuiltCore=mynetlist2.edf}". You would then create a VHDL package file. This would comprise of a VHDL package containing the core's component declaration. You would then be able to include this core for any worker using ``\code{Cores=<core-name>}''.

\section{Simulating Vivado IP or PreBuilt Cores with XSIM in ANGRYVIPER}
\label{sim_ip}
You may be able to build for XSIM with ANGRYVIPER by including the stub vhdl file mentioned in \ref{prebuiltcore}, but omitting the netlist. If this does not work, you will have to generate a simulation netlist. \newline
...
\section{Using Vivado IP with ANGRYVIPER}
To use Vivado's IP within ANGRYVIPER, you can follow these steps:
\begin{itemize}
...
\subitem  \textit{Note}: Reference section \ref{prebuiltcore} for more information on using Vivado pre-built cores with ANGRYVIPER
...
%\item Note that we recommend placing pre-built cores in %ANGRYVIPER Primitive Cores. Doing so involves creating a %\code{<ip\_name>\_pkg.vhd} file which would contain a VHDL %package with the core's component declaration. This package %file replaces the stub file.
...
Prior to running any vivado/xsim commands, you must source \code{<path-to-vivado>/settings64.sh}. Because these settings interfere with ANGRYVIPER's environment, you should always do this in a separate terminal.
...
For example, for the complex\_mixer.test Unit Test, after running case00.00 with KeepSimulations=1, you can run:\newline
...
\section{ANGRYVIPER Output Files for Vivado}
\begin{itemize}
\item \code{.jou}: Vivado journal file
\item \code{.jou.bak}: Backup of the previously generated Vivado journal file
\item \code{<asset-name>-vivado.out}: ANGRYVIPER \textit{and} Vivado output for synthesis of an asset
\item \code{<impl-stage>.out}: ANGRYVIPER \textit{and} Vivado output for a stage of implementation
\item \code{.xpr}: Every stage of compilation for every ANGRYVIPER asset results in a Vivado project file. This project file can be opened in Vivado to observe the source files associated with that stage.
...
\item \code{.libs}, \code{.sources}, \code{.cores}: The ANGRYVIPER files used to store information regarding the libraries, sources, and cores that an asset depends on
...
\end{itemize}
\section{ANGRYVIPER Output Files for XSIM}
\begin{itemize}
