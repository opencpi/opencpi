...
\section{Enhancements}
\subsection{New Digital Radio Controller Front-End Control Paradigm} % AV-3840
\label{sec:15_drc}
This new OpenCPI Specification File (OCS) represents a ``\textit{digital radio controller}.''
...
\textit{data stream}'s samples, \textit{e.g.} tuning
frequency.\\
...
\label{bug:5275}
Starting with 1.4, some RPMs pre-install symlinks for rules in \path{/etc/udev/rules.d/} for certain radio platforms, \textit{e.g.} a rule to create a symlink from \path{/dev/matchstiq_z1_0} to \path{/dev/ttyUSB0}. If the OpenCPI installation location is on a different partition than the host machine's root partition, the symlinks for these rules may be broken at boot-time because the target partition is not yet mounted when \path{udev} is reading the rules. To fix this, the command ``\code{sudo udevadm control --reload}'' should be executed upon a full boot. With CentOS~7, a \href{https://askubuntu.com/a/679600/139131}{custom \path{systemd} service can be implemented}.
...
\item \bugref{1035}
\item \bugref{1392}
\item \bugref{2241} %% MIGHT BE FIXED FINAL RELEASE
\item \bugref{2400}
\item \bugref{2477}
\item \bugref{3193}
\item \bugref{3783}
