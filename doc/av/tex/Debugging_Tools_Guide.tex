...
%There is now a remote connection to the debug server on the embedded platfrom.  The user will use the same commands as in the previous section (debugging using gdb command line) to debug a rcc worker.
...
Once the synthesis completes, you will need to open the resulting checkpoint (found in the project sources), and use \code{write\_edif} or \code{write\_checkpoint} write these out in a format that can be used later:
\begin{itemize}
		\item Enter the Tcl Console, and overwrite the netlist created by the `make' system in the ``target-zynq'' directory:
			\subitem \code{> write\_edif vivado\_ila.edf}
			\subitem \textit{or}
		...
		\item In the Tcl Console, generate the *.ltx file containing the debug probe information:
			\subitem \code{> write\_debug\_probes vivado\_ila.ltx}
\end{itemize}
Note that some probe names may be unhelpful unless the \code{flatten\_hierarchy} option is set to ``none'' during synthesis of the asset being debugged (in this case the worker). This can be done either in the Vivado GUI or via an ANGRYVIPER Makefile option as explained in \textit{ANGRYVIPER\_Vivado\_Usage.pdf}.\newline
...
Note: ISE debug cores (NGCs) can be used in conjunction with Chipscope for debugging \textit{even if Vivado is the tool that ANGRYVIPER is using} to synthesize and implement designs. Reference \ref{ise1} for information on including NGC debug cores.
	...
	\begin{enumerate}
		\item Build the worker:
			\subitem \code{> cd ocpiassets/components/dsp\_comps/complex\_mixer.hdl}
			\subitem \code{> make HdlTarget=zynq VivadoExtraOptions\_synth="-flatten\_hierarchy none"}
			\subsubitem Note that some probe names may be unhelpful unless the \code{flatten\_hierarchy} option is set to ``none''
		...
		\item Open up the worker's Vivado project:
			\subitem \code{> cd target-zynq}
			\subitem \code{> (source /opt/Xilinx/Vivado/2017.1/settings64.sh ; vivado complex\_mixer.xpr) \&}
			...
			\subitem set the \code{flatten\_hierarchy} option to \code{none} via the GUI as well.
		...
		\item Rerun synthesis. Note that you may once again want to set the \code{flatten\_hierarchy} option set to ``none'' via the GUI. Observe the debug cores in the worker's netlist:
			...
			\end{figure}
		\item Enter the Tcl Console, and overwrite the netlist created by the `make' system in the ``target-zynq'' directory:
			\subitem \code{> write\_edif -force complex\_mixer.edf}
