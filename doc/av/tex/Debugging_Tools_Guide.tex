...
\end{itemize}
Note that some probe names may be unhelpful unless the \code{flatten\_hierarchy} option is set to ``none'' during synthesis of the asset being debugged (in this case the worker). This can be done either in the Vivado GUI or via an OpenCPI Makefile option as explained in \textit{OpenCPI\_Vivado\_Usage.pdf}.\newline
