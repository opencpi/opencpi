...
\section{Functionality}
  ...
  \subsection{Digital Radio Controller Concept and Definition}
    ...
    \textit{data streams} at once as well as performing multiple successive locks
    and unlocks. Each implementation of this component spec represents a
    single \textit{digital radio controller}.
  ...
  \subsection{Data Stream Concept and Definition}
    ...
    \begin{itemize}
      \item It corresponds to an RF data sink or source on the
            \textit{digital radio},
            usually an SMA connector.
      \item It has an associated \textit{data stream ID}, which usually
            corresponds to the
            label of the SMA connector it represents.
      \item It can be configured with at least one \textit{data stream type}
            ...
            configuring an SMA connector for either RX or TX.
      \item It has at a minimum configurations for tuning freq, 3dB bandwidth,
  ...
  \subsection{Supporting C++ Classes}
    ...
    \begin{itemize}
      ...
      \item \textit{Configurator} - Software-only emulator of a hardware configuration environment which provides an API for locking/unlocking range-constrained configs. Each config has a constraint which is a valid range represented by an object of the \textit{ValidRanges} type.
