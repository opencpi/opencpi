trap "trap - ERR; break" ERR; for i in 1; do
. ./env/start.sh

export OCPI_XILINX_DIR=/opt/Xilinx
export OCPI_XILINX_VERSION=14.6
export OCPI_XILINX_LICENSE_FILE=/home/jek/mac/Xilinx/Xilinx-License.lic
. ./env/xilinx.sh

export OCPI_ALTERA_DIR=/home/jek/mac/Altera
export OCPI_ALTERA_VERSION=12.1
export OCPI_ALTERA_LICENSE_FILE=$OCPI_ALTERA_DIR/1-99VGI3_License.dat
. ./env/altera.sh

export OCPI_BUILD_SHARED_LIBRARIES=0
. ./platforms/centos6/centos6-env.sh

# #### Location of the Modelsim tools ####################################### #

export OCPI_MODELSIM_DIR=/home/jek/mac/Mentor/modelsim_dlx
export OCPI_MODELSIM_LICENSE_FILE=$OCPI_MODELSIM_DIR/../license-20151005.txt

# Enable this if you want to, and have, installed OPENCV
# export OCPI_OPENCV_HOME=/opt/opencpi/prerequisites/opencv/linux-c6-x86_64

export OCPI_HAVE_OPENCL=1
export OCPI_OPENCL_OBJS=/opt/opencpi/prerequisites/amd-sdk/linux-c6-x86_64/lib/x86_64/libOpenCL.so
export OPENCL_VENDOR_PATH="/opt/opencpi/prerequisites/amd-sdk/installed/AMDAPPSDK-3.0/etc/OpenCL/vendors/"
. ./env/finish.sh
done; trap - ERR
